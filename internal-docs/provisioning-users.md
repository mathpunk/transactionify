...
This guide will help you provision a new user. But first, let's clarify **which users we're talking about**.

The Transactionify service empowers developers to create accounts, post payments to accounts, and check the balances of those accounts. The users of those accounts, we'll call **customer users**. Those IDs are managed by consumers of the Transactionify API, and they are created via API gateway through the `/accounts` endpoint. See [openapi.yaml](../openapi.yaml), [the Postman collection](../Transactionify_API.postman_collection.json), or the [public documentation](https://transactionify-th.readme.io/reference/createaccount) for more on customer users.

The other sort of user is a developer who is consuming the Transactionify API. We'll call these users **client users**. These users need API keys in order to access Transactionify. But we do **not** expose the functionality for creating these client users through an API gateway. Instead, that functionality is secured behind an AWS Lambda, and invoked via command line, the AWS Console, or automation scripts.

In this guide, we'll assume you're creating a new client user from the command line. The procedure should be similar for automated scripts, provided the host machine has the necessary prerequisites (although, read the note about IAM permissions). 
...
You can supply an empty dictionary as a payload (`'{}'`), and a user_id for the client user will be automatically generated. 
...
You can also provide your own; however, the only acceptable format for a user_id is **UUIDv7** (which includes a timestamp for sortability): 
...
If you supply a user_id in a different format (such as UUIDv4), the Lambda will reject it with a 400 error, like so:
...
After running your command, check the response file:
...
The `api_key` in the response is what you'll provide to the client for authentication. **Keep it secret!** Our client developers will use it in the `Authorization` header like this:

```bash
Authorization: APIKey 019a4757-c049-7ea8-a110-2ea110c5a6f6
```
If client users need more help on using their keys to make requests, you can direct them to [the quickstart in our public documentation](https://transactionify-th.readme.io/docs/quickstart). 
...
By requiring AWS credentials to invoke the Lambda function, and not exposing the function to the open web, we add an extra layer of security for client user creation.
